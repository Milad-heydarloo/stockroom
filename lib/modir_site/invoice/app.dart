...
class MyApp_invoce extends StatefulWidget {
  OrderTwo? us;
List<ProductAtwo> listprodut;
  MyApp_invoce({Key? key, required this.us,required this.listprodut}) : super(key: key);
...
class MyApp_invoceState extends State<MyApp_invoce>
  ...
  late List<ProductInvo> products;
...
products=[];
  ...
  //  final users = await GoogleSheetsControllerproductorder.getAllproductorder();
   ...
   // print(filteredUsers.length);
    ...
    products = widget.listprodut!.map((user) {
      final product = ProductInvo(
        _counter.toString(),
        user.title!,
      ...
      body: PdfPreview(
        ...
        build: (format) {
          ...
          Future.delayed(Duration(seconds: 3), () {
            Future<Uint8List> result = generateInvoice(
